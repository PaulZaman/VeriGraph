...
class ModelLoader:
    def __init__(self, model_stage=None):
        self.checker = None
    ...
    def initialize(self):
        ...
        try:
            ...
            try:
                ...
                # Try components/model (MLflow transformers format) - MOST COMMON
                if os.path.exists(os.path.join(model_path, "model", "config.json")):
                    model_dir = os.path.join(model_path, "model")
                    # Check if tokenizer is in components/tokenizer (standard MLflow format)
                    if os.path.exists(os.path.join(model_path, "components", "tokenizer", "tokenizer_config.json")):
                        tokenizer_dir = os.path.join(model_path, "components", "tokenizer")
                    else:
                        # Fallback: tokenizer in same directory as model
                        tokenizer_dir = model_dir
                    logger.info(f"✓ Detected MLflow transformers format (model/ + components/tokenizer/)")
                # Try components/model (alternative structure)
                elif os.path.exists(os.path.join(model_path, "components", "model")):
                    model_dir = os.path.join(model_path, "components", "model")
                    tokenizer_dir = os.path.join(model_path, "components", "tokenizer")
                    logger.info("✓ Detected MLflow transformers format (components/model/)")
                # Try flat structure
                elif os.path.exists(os.path.join(model_path, "config.json")):
                    model_dir = model_path
                    tokenizer_dir = model_path
                    logger.info("✓ Detected flat model structure")
                else:
            ...
            # Fallback: Manual download approach using direct API
            # Setup cache directory and download model
            model_cache_path = os.path.join(self.cache_dir, run_id)
            
            # For MLflow transformers models, we need the components structure
            expected_paths = [
                os.path.join(model_cache_path, "components", "model", "config.json"),
                os.path.join(model_cache_path, "components", "tokenizer", "tokenizer_config.json")
            ]
            ...
            if all(os.path.exists(p) for p in expected_paths):
                ...
                # Download artifacts with extended timeout
                logger.info(f"📥 Downloading model artifacts from run {run_id[:8]}...")
                logger.info("This may take 2-3 minutes for large models...")
                os.makedirs(model_cache_path, exist_ok=True)
                
                # Download with MLflow client
